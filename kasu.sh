...
gum spin --spinner line --title "🛜 Installing curl, turning swap off, setting ip-forwarding on..." -- apt update && # && apt upgrade -y
...
gum spin --spinner line --title "☸️ Installed kubelet kubeadm kubectl, version 1.28" -- curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.28/deb/Release.key | gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg &&
...
gum spin --spinner line --title "📦 Pulling k8s images" --show-output -- kubeadm config images pull
...
if [ MODE == "control-plane" ]; then
    ...
    echo "✅ Done! If you didn't use '&& kubeadm join YOUR_TOKENS_HERE', you can do so now."
fi